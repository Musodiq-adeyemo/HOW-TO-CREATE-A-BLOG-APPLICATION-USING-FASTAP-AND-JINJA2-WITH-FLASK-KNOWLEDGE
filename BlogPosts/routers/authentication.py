...
def login(request:OAuth2PasswordRequestForm=Depends(),Authorize:AuthJWT=Depends(), db:Session=Depends(get_db)):
    ...
    if (request.username == user.username and verify_password):
        access_token = Authorize.create_access_token(subject=request.username,expires_time=access_token_expire)
        refresh_token = Authorize.create_refresh_token(subject=request.username,expires_time=refresh_token_expire)
        
        Authorize.set_access_cookies(access_token)
        Authorize.set_refresh_cookies(refresh_token)
        return {f"You have been login successfully"}
