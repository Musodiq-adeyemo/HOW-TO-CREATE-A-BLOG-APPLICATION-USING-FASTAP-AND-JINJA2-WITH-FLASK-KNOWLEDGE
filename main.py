...
from BlogPosts.schemas import Settings
...
def load_user(username:str,db:Session=Depends(get_db)):
    current_user = db.query(User).filer(User.username==username).first()
    return current_user
...
def signin(request: Request,username:str=Form(...),password:str=Form(...),db:Session = Depends(get_db)):
    errors = []
...
def dashboard(request: Request,db:Session = Depends(get_db)):
    ...
    users = db.query(User).all()
    images = db.query(ProfileImage).all()
    return templates.TemplateResponse("dashboard.html",{"request":request,"profile":profiles,"users":users,"images":images})

#@app.get("/dashboard/{{profile_id}}",response_class=HTMLResponse,tags=["Template"])
#def update_post(request: Request,profile_id:int,db:Session=Depends(get_db)):
    #profile = db.query(Profile).filter(Profile.id == profile_id).first()
    #return templates.TemplateResponse("dashboard.html",{"request":request,"profile":profile})

@app.post("/dashboard/{{profile_id}}",response_class=HTMLResponse,tags=["Template"])
...
def upload_pimage(request: Request):
...
def users(request: Request, db:Session = Depends(get_db),current_user=Depends(manager)):
    users = db.query(User).all()
    profiles = db.query(Profile).all()
    images = db.query(ProfileImage).all()
    current_user = manager.get_current_user()
...
def upload_postimage(request: Request):
...
def comment(request: Request, db:Session = Depends(get_db),username:str=Form(...),email:str=Form(...),message:str=Form(...)):
    errors = []
    try:
...
def message(request: Request, db:Session = Depends(get_db),user=Depends(manager)):
    user = manager.get_current_user()
    messages = db.query(Message).all()
    return templates.TemplateResponse("message.html",{"request":request,"messages":messages,"user":user})
